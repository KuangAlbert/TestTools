/**
 * Read MFI ID
 * Modified by Xie Qihuai 2015-09-15 for DS03
 */
...
#define DEVICE_VERSION		0x05	/* MFI 2.0C R1 */

char *ipod_reset = "/sys/class/gpio/IPOD_RESET/value";
char *i2c_ipod = "/dev/i2c-ipod";
static int fd_reset = -1;
static int fd_i2c = -1;

static int ipod_ic_reset(int value)
{
	int ret;
	char buf[2];
...
int main(int argc, char** argv)
{
	...
	usleep(300000);
	ipod_ic_reset(1);
	/* on MFI IC data sheet, we know that reset I/O pin must remain the same for at least 30ms.
