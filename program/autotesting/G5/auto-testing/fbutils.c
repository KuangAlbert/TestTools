...
void put_char(int x, int y, int c, int colidx)
{
	...
	for (i = 0; i < font_12x22.height; i++) {
		bits = font_12x22.data [font_12x22.height * c + i];
		for (j = 0; j < font_12x22.width; j++, bits <<= 1)
			if (bits & 0x8000)
...
{
	int i;
	for (i = 0; *s; i++, x += font_12x22.width, s++)
...
{
	size_t sl = strlen (s);
        put_string (x - (sl / 2) * font_12x22.width,
                    y - font_12x22.height / 2, s, colidx);
}
