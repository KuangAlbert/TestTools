...
static struct fbcon_font_desc *font =  &font_12x22;
/* struct fbcon_font_desc *font =  &font_8x8; */
...
void fb_open(int fb_num, struct fb_info *fb_info)
{
	...
	/*mediatek need to do pan display to init hardware*/
	fb_info->var.xoffset = 0;
	fb_info->var.yoffset = 0;
	IOCTL1(fd, FBIOPAN_DISPLAY, &fb_info->var);
		...
		int color, int clear, int clearlen)
{
	...
	if (clear)
		fb_clear_area(fb_info, x, y, clearlen * font->width, font->height);

	for (i = 0; i < strlen(s) && i < maxlen; i++) {
		fb_put_char(fb_info, (x + font->width * i), y, s[i], color);
		w += 8;
	}
...
void put_string_center(struct fb_info *fb_info, int y, char *s, int color)
{
	size_t sl = strlen(s);
	int x;
	x = (fb_info->var.xres/2) - (sl/2 * font->width);
	fb_put_string(fb_info, x, y, s, sl, color, 0, 0);
}
...
void put_string_info(struct fb_info *fb_info, char *buf, int i, int color)
{
	...
	put_string_center(fb_info, 150+y, buf, color);
}
